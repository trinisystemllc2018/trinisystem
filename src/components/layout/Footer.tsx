...
export function Footer() {
  return (
    <footer className="t-bg-2 t-text" style={{ borderTop: "1px solid var(--border)" }}>
      {/* CTA Strip */}
      <div className="py-12 px-4" style={{ background: "linear-gradient(135deg, var(--primary), var(--primary-2))", color: "var(--on-primary)" }}>
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="text-2xl md:text-3xl font-black mb-1">
              Printer broken? PC slow? We fix it today.
            </h2>
            <p style={{ opacity: 0.85 }}>24/7 remote support · All brands · No fix, no fee</p>
          </div>
          <div className="flex gap-3 shrink-0 flex-wrap justify-center">
            <a
              href={PHONE_HREF}
              className="font-bold px-6 py-3.5 rounded-2xl transition-transform hover:scale-[1.03] shadow-lg" style={{ background: "var(--surface-solid)", color: "var(--primary)" }}
            >
              📞 {PHONE}
            </a>
            <Link
              href="/contact"
              className="font-bold px-6 py-3.5 rounded-2xl transition-transform hover:scale-[1.03]" style={{ background: "rgba(0,0,0,0.25)", color: "#fff", border: "1px solid rgba(255,255,255,0.35)" }}
            >
              Book Online →
            </Link>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-12 mb-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-2.5 mb-4">
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-white font-black text-sm">
                TS
              </div>
              <span className="font-bold text-lg t-text">Trini System <span style={{color:"var(--accent)"}}>LLC</span></span>
            </div>
            {/* 8-year badge */}
            <div className="flex items-center gap-2 mb-3">
              <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: "var(--primary-soft)", color: "var(--primary)", border: "1px solid var(--border-strong)" }}>
                🏆 Est. 2016 — 8 Years in Business
              </span>
            </div>
            <p className="t-muted text-sm leading-relaxed mb-4 max-w-xs">
            ...
            <address className="not-italic mb-4 space-y-1">
              <a href={PHONE_HREF} className="text-xl font-bold transition-colors flex items-center gap-2">
                📞 {PHONE}
              </a>
              <p className="text-xs t-faint">52-09 99th St Apt 8S, Corona, NY 11368</p>
              <p className="text-xs t-faint">Available 24/7 · Remote & On-Site</p>
            </address>
            {/* Google rating link */}
            <a
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <a href={SOCIAL.facebook} target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-xl bg-orange-600 hover:bg-orange-500 flex items-center justify-center transition-colors" aria-label="Facebook">
          ...
          {footerSections.map(section => (
            <div key={section.title}>
              <h3 className="text-xs font-bold uppercase tracking-widest t-faint mb-4">
                {section.title}
              </h3>
                  ...
                  <li key={link.label}>
                    {"external" in link && link.external ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-sm t-muted hover:opacity-100 transition-opacity" style={{}}
                      >
                        {link.label} ↗
                      </a>
                    ) : (
        ...
        <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4" style={{ borderTop: "1px solid var(--border)" }}>
          <p className="text-xs t-faint text-center md:text-left">
          ...
          <div className="flex items-center gap-3">
            {["24/7 Available", "Est. 2016", "All 50 States", "4.9★ Google"].map(b => (
              <span key={b} className="text-xs t-muted px-3 py-1 rounded-full" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                {b}
              </span>
            ))}
          </div>
